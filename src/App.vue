...
const upViewList = [
  { key: 'Dashboard', label: '基本情報', icon: 'material-symbols:dashboard' },
  { key: 'Drop', label: 'ドロップ情報', icon: 'material-symbols:bookmark-star-sharp' },
]
const downViewList = [
...
async function handleClick() {
  ...
  try {
    ...
    await result.download({ quality: 3, scale: 3, type: 'png', filename: `設定スクリーンショット${Date.now()}` })
  }
  catch (error) {
    console.error(error)
  }
  ...
  <div fc flex-col flex-wrap gap-3 p-10>
    ...
    <div h-600px w-600px ring-1>
      <div fc gap-2 p-2>
        ...
        <TheButton title="テスト" @click="handleClick">
          ボタン
        </TheButton>
      </div>
    </div>
