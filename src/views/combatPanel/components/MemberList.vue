...
import type { MemberInfo } from 'battle'
import { battleInfo, combatPanelSetting, markedUserList, userInfo } from '~/logic'
...
const mvpInfo = computed(() => battleInfo.value.mvpInfo)
...
const data = computed(() =>
  memberInfo.value?.reduce<MemberInfo[]>((pre, cur) => {
    const hit = mvpInfo.value?.find(m => m.userId === cur.userId)
...
function handleMark(member: MemberInfo) {
  ...
  if (hit) {
    ...
    ElMessage.info('このプレイヤーは既にブックマークされています。ニックネームとRankを更新しました')
  }
  else {
    markedUserList.value.unshift({
      ...
      rate: 0,
      comment: `ブックマーク日: ${useDateFormat(Date.now(), 'YYYY-MM-DD HH:mm:ss').value}`,
    })
    ElMessage.success('プレイヤーをブックマークしました')
  }
}
</script>
...
<template>
  <UseDraggable
    ...
    @end="(position) => {
      combatPanelSetting.MemberList.x = position.x
