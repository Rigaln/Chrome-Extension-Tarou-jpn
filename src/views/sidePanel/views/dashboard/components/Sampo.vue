...
<template>
  <el-descriptions v-if="sampoInfo.areaName" :column="3" :border="true" direction="vertical" w-300px size="small">
    <el-descriptions-item label="探検隊情報" align="center" :span="3">
      <template #label>
        <div flex justify-between>
          <el-tooltip content="探検隊の装備を開く" placement="top">
            <div cursor-pointer text-12px text-teal-6 hover:text-teal-4 @click="openPopupWindow('SampoSetup')">
              {{ `探検隊 Lv${sampoInfo.level}` }}
            </div>
          </el-tooltip>

          <el-tag v-if="sampoInfo.areaName" :type="sampoInfo.isFinished ? 'success' : 'danger' ">
            {{ sampoInfo.isFinished ? '待機中' : '探索中' }}
          </el-tag>
        </div>
      </template>
      {{ sampoInfo.areaName }}
    </el-descriptions-item>
    <el-descriptions-item label="現在/終了" align="center" label-width="100">
      <el-tooltip content="現在のやる気/探索終了時のやる気">
        {{ sampoInfo.maxStamina ? `${sampoInfo.currentStamina}/${sampoInfo.isFinished ? '-' : sampoInfo.endStamina}` : '-' }}
      </el-tooltip>
    </el-descriptions-item>
    <el-descriptions-item label="探索カウントダウン" align="center" label-width="100">
    ...
    <el-descriptions-item label="やる気全回復" align="center" label-width="100">
  ...
  <el-alert v-else title="探索情報が取得されていません" type="info" :center="true" :closable="false" />
</template>
